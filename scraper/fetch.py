#!/usr/bin/env python3
"""
Denton County TX – Motivated Seller Lead Scraper
Clerk  : denton.tx.publicsearch.us
CAD    : dentoncad.net → 2026preliminaryDataReal.zip (fixed-width)
"""
...
GRANTEE_IS_OWNER = {"LC", "LN", "CSL", "FTL", "STL", "NLF", "JUD", "AJ", "ALN", "HL"}
...
ENTITY_FILTERS = (
    ...
    "N/A", "UNKNOWN", "PUBLIC", "ATTY GEN", "ATTY/GEN", "DENTON CITY",
    "CITY DENTON", "DENTON COUNTY"
)
...
def build_parcel_lookup() -> dict:
    lookup = {}
    log.info("Downloading Denton CAD data ...")
    try:
        ...
        for line in raw.splitlines():
            ...
            parcel = {
                ...
                "prop_city":    situs_city or "Denton",
                "prop_state":   "TX",
                "prop_zip":     situs_zip,
        ...
        log.info(f"Denton CAD lookup built: {len(lookup):,} name variants from {total:,} parcels")
    except Exception:
...
def score_record(rec: dict) -> tuple:
    ...
    if dtype in ("LC","LN","ALN"):     flags.append("Lien")
...
def build_output(raw_records: list, date_from: str, date_to: str) -> dict:
    ...
    return {
        ...
        "source":       "Denton County TX – PublicSearch",
...
async def main():
    ...
    log.info("=== Denton County TX Lead Scraper ===")
